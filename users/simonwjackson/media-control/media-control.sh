...
send_mpv_command() {
  case "$1" in
    "XF86AudioPlay")
      echo '{ "command": ["cycle", "pause"] }' | socat - UNIX-CONNECT:"$MPV_SOCKET"
      ;;
    "XF86AudioNext")
      echo '{ "command": ["playlist-next"] }' | socat - UNIX-CONNECT:"$MPV_SOCKET"
      ;;
    "XF86AudioPrev")
      echo '{ "command": ["playlist-prev"] }' | socat - UNIX-CONNECT:"$MPV_SOCKET"
      ;;
    esac
}
