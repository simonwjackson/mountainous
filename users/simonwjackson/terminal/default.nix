...

{
  ...
  programs.git = {
    ...
    aliases = {
      ...
      gpt = "!f() { git diff --staged | sgpt --model gpt-4 \"Generate a conventional commit from these chages. template: ':emoji: <type>[optional scope]: <description>\n\n[optional body]\n\n[optional footer(s)]'. prepend first line with a gitmoji related to the type. ideally, first line is 50 chars or less\" | tee /dev/tty | (git commit -F - --edit || true); }; f";
