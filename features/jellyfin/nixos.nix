  ...
  config = mkIf cfg.enable (mkMerge [
    {
      assertions =
        ...
        ++ optional (!mediaCfg.enable) {
          assertion = false;
          message = "mountainous.features.jellyfin requires mountainous.features.media.enable = true";
        }
        ++ optional (cfg.port != 8096) {
          assertion = false;
          message = "mountainous.features.jellyfin currently supports only port 8096";
        }
        ++ optional cfg.proxy.enable {
          assertion = tsnetProxyEnabled;
          message = "mountainous.features.jellyfin requires mountainous.features.tsnet-proxy.enable = true when proxy.enable = true";
        }
        ++ optional cfg.bootstrap.enable {
          assertion = cfg.bootstrap.admin.passwordFile != null;
          message = "mountainous.features.jellyfin requires bootstrap.admin.passwordFile when bootstrap.enable = true";
        }
        ++ optional cfg.watchedCleaner.enable {
          assertion = cfg.bootstrap.enable && cfg.bootstrap.admin.passwordFile != null;
          message = "mountainous.features.jellyfin.watchedCleaner requires bootstrap.enable = true with a passwordFile for admin credentials";
        };
