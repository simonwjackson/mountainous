  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      systemd.services.radarr-seed-jellyfin-notification = mkIf cfg.notifications.jellyfin.enable {
        description = "Seed Radarr Jellyfin/Emby notification connection";
        after =
          ["radarr.service"]
          ++ optional cfg.vpn.enable "vpn-ns.service";
        requires =
        ...
        script = ''
          ...
          def authenticate_jellyfin():
              auth_header = 'MediaBrowser Client="mountainous", Device="mountainous", DeviceId="radarr-jellyfin", Version="1.0.0"'
              payload = json.dumps({
          ...
          if existing_notif is None:
              request_json("/api/v3/notification?forceSave=true", method="POST", data=template)
              print(f"Created Radarr notification: {notification_name}")
          else:
              template["id"] = existing_notif["id"]
              request_json(
                  f"/api/v3/notification/{existing_notif['id']}?forceSave=true",
              ...
              print(f"Updated Radarr notification: {notification_name}")
          PY
        '';
      };
