{
  ...
  cfg = config.services.aria2-custom;
...
in {
  imports = [
    (lib.mkRemovedOptionModule ["services" "aria2-custom" "rpcSecret"] "Use services.aria2.rpcSecretFile instead")
    (lib.mkRemovedOptionModule ["services" "aria2-custom" "extraArguments"] "Use services.aria2.settings instead")
    (lib.mkRenamedOptionModule ["services" "aria2-custom" "downloadDir"] ["services" "aria2-custom" "settings" "dir"])
    (lib.mkRenamedOptionModule ["services" "aria2-custom" "listenPortRange"] ["services" "aria2-custom" "settings" "listen-port"])
    (lib.mkRenamedOptionModule ["services" "aria2-custom" "rpcListenPort"] ["services" "aria2-custom" "settings" "rpc-listen-port"])
  ];
  ...
  options = {
    services.aria2-custom = {
      ...
      user = lib.mkOption {
        type = lib.types.str;
        default = "aria2";
        description = "User account under which aria2 runs.";
      };
      # Add new group option
      group = lib.mkOption {
        type = lib.types.str;
        default = "aria2";
        description = "Group under which aria2 runs.";
      };
  ...
  config = lib.mkIf cfg.enable {
    ...
    networking.firewall = lib.mkIf cfg.openPorts {
      allowedUDPPortRanges = config.services.aria2-custom.settings.listen-port;
      allowedTCPPorts = [config.services.aria2-custom.settings.rpc-listen-port];
    };

    users.users = lib.mkIf (cfg.user == "aria2") {
      aria2 = {
        group = cfg.group;
    ...
    users.groups = lib.mkIf (cfg.group == "aria2") {
      aria2.gid = config.ids.gids.aria2;
    };

    systemd.tmpfiles.rules = [
      "d '${homeDir}' 0770 ${cfg.user} ${cfg.group} - -"
      "d '${config.services.aria2-custom.settings.dir}' 0770 ${cfg.user} ${cfg.group} - -"
    ];

    systemd.services.aria2-custom = {
