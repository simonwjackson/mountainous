...
class AutoRotate:
    ...
    def _validate_devices(self):
        ...
        if not self.accelerometer_device.exists():
            raise FileNotFoundError(
                f"Accelerometer device not found: {self.accelerometer_device}"
            )
    ...
    def _read_accelerometer(self) -> Optional[SensorReading]:
        ...
        try:
            ...
            return SensorReading(x=float(x_raw), y=float(y_raw), z=float(z_raw))
    ...
    def _apply_rotation(self, orientation: int):
        ...
        try:
            ...
            self.logger.info(
                f"Rotated display to {orientation_info.name} (transform {orientation_info.transform}, layout: {layout_desc})"
            )
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                self.logger.debug(
                    f"Accel: X={accel_reading.x}, Y={accel_reading.y}, Z={accel_reading.z}"
                )

                # Check if significant movement detected
                if (
                    abs(accel_reading.x) < self.rotation_threshold
                    and abs(accel_reading.y) < self.rotation_threshold
                ):
                    self.logger.debug(
                        f"Below threshold - X:{abs(accel_reading.x)} < {self.rotation_threshold}, Y:{abs(accel_reading.y)} < {self.rotation_threshold}"
                    )
                    time.sleep(self.poll_interval)
                    continue
                ...
                self.logger.debug(
                    f"Calculated orientation: {new_orientation} (current: {self.current_orientation})"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Automatic screen rotation for Lenovo Fold"
    )
    parser.add_argument(
        "--accelerometer",
    ...
    parser.add_argument(
        "--debounce-time", type=int, default=2, help="Debounce time in seconds"
    )
    parser.add_argument(
        "--hyprland-instance", type=int, default=0, help="Hyprland instance number"
    )
    parser.add_argument(
        "--poll-interval", type=float, default=0.5, help="Sensor polling interval"
    )
